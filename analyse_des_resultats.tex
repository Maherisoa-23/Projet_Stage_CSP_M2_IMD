...
\begin{itemize}[leftmargin=2em,itemsep=0.2em]
  \item \texttt{always\_planar}~: 100\% des runs plans \emph{et} $\mu_{\text{angle}} < 5^\circ$.
  \item \texttt{mostly\_planar}~: $\geq 70\%$ plans \emph{et} $\sigma < 3^\circ$.
  \item \texttt{unstable}~: 30--70\% plans \emph{ou} $\sigma > 5^\circ$.
...
\begin{itemize}[leftmargin=2em,itemsep=0.2em]
  \item \textbf{200 originaux tout-hexagonal} (= benzénoïdes BenzAI sans substitution)~: 100\% planar, angle maximum 0,87$^\circ$.
...
Sur les 91 solutions classées \texttt{unstable} (h4 + h5), \textbf{toutes les 91 ($100\%$)} présentent une distribution des 10~angles ACP en \emph{deux groupes nettement séparés}, avec un écart vide entre les deux d'au moins 5$^\circ$. Aucune n'est uniformément distribuée autour du seuil de 10$^\circ$.

\textbf{Exemples concrets} (les 10 angles d'une solution unstable)~:
\begin{itemize}[leftmargin=2em,itemsep=0.2em]
  \item \texttt{h5 / 2-5-6-7-13 [v0=6 v1=7 v2=7 v3=5 v4=5]}~:
    angles plans = $\{0{,}1^\circ;\ 0{,}3^\circ\}$,
    angles courbés = $\{30{,}8^\circ;\ 30{,}8^\circ;\ 30{,}8^\circ;\ 30{,}9^\circ;\ 30{,}9^\circ;\ 30{,}9^\circ;\ 31{,}2^\circ;\ 31{,}5^\circ\}$.
    Écart vide = 30,5$^\circ$.
  \item \texttt{h5 / 2-5-6-7-8 [v0=7 v1=7 v2=6 v3=5 v4=5]}~:
    8 runs aux alentours de 0,3$^\circ$, 2 runs aux alentours de 29$^\circ$.
    Écart vide = 28$^\circ$.
\end{itemize}
...
Le mot \og unstable\fg{} évoque naturellement un cas \og incertain numériquement \fg{}~: on imaginerait une molécule dont l'angle \emph{vrai} est aux alentours de 10$^\circ$, et le bruit numérique fait qu'un run la classe à 9$^\circ$ (plan) et un autre à 11$^\circ$ (non-plan). Augmenter $N$ corrigerait alors l'imprécision.
...
\begin{itemize}[leftmargin=2em,itemsep=0.2em]
  \item un conformère \emph{plat} (angle ACP $\approx 0^\circ$), à plus basse énergie ou comparable, qu'on attend pour les molécules aromatiques planes,
  \item un conformère \emph{plié / courbé} (angle ACP $\approx 25\text{--}35^\circ$), où la molécule s'organise en une surface non plane (en forme de bol ou de selle), correspondant à un autre minimum local.
\end{itemize}
...
\[
\alpha(n) = \frac{(n-2) \cdot 180$^\circ$}{n}
\quad \Rightarrow \quad
\alpha(5) = 108$^\circ$, \quad \alpha(6) = 120$^\circ$, \quad \alpha(7) \approx 128{,}57$^\circ$.
\]
Par rapport à un hexagone régulier (référence d'un pavage plan parfait), un pentagone introduit un \emph{défaut angulaire} de $-12^\circ$ par sommet (la chaîne se concave, comme dans un fullerène), et un heptagone introduit un défaut de $+8{,}57^\circ$ par sommet (la chaîne s'évase, comme dans un graphène hyperbolique).
...
\begin{itemize}[leftmargin=2em,itemsep=0.3em]
  \item \textbf{Bimodal} (= 91 solutions \texttt{unstable})~: deux groupes nets ($\sim 0^\circ$ et $\sim 25\text{--}35^\circ$). Ce ne sont \emph{pas} des borderline. $N=30$ ne change pas la classe, juste la probabilité estimée.
  \item \textbf{Borderline} (= ${\sim}21$ solutions identifiées)~: les angles individuels sont \emph{tous} entre $7^\circ$ et $13^\circ$, autour du seuil de 10$^\circ$. Pas de bimodalité, juste de la dispersion près du seuil. \textbf{C'est ici que $N=30$ aurait du sens}.
\end{itemize}
...
\end{itemize}
Ici on voit clairement que les angles \og non-plans \fg{} sont $\approx 9^\circ$, juste sous le seuil~: pas de second bassin physique, juste une dispersion dont la queue chatouille le seuil. $N=30$ donnerait un classement stable.

\subsection{Liste prioritaire pour re-runs ciblés}

\begin{enumerate}[leftmargin=2em,itemsep=0.3em]
  \item \textbf{21 borderlines} ($\mu \in [7^\circ, 13^\circ]$ et \emph{non} \texttt{unstable})~: re-run $N=30$. Coût $= 630$ runs. Permet de trancher si vraiment près du seuil. \textbf{Priorité 1}.
  \item \textbf{4 \texttt{always\_non\_planar} avec $\mu < 15^\circ$}~: vérifier qu'ils ne sont pas en réalité bistables avec un bassin plan rare. \textbf{Priorité 2}.
...
\begin{enumerate}[leftmargin=2em,itemsep=0.3em]
  ...
  \item \textbf{Le seuil $10^\circ$} est cohérent avec la bimodalité globale (creux entre $1^\circ$ et $28^\circ$), mais arbitraire pour les ${\sim}20$ cas borderline. Un seuil empirique à $5^\circ$ serait plus défendable mathématiquement.
