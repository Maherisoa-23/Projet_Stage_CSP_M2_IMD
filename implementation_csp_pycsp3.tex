...
1203 configurations locales validées par optimisation xTB (GFN2-xTB) et test de planarité
(seuil 10°).
...
selon la nature de l'hexagone :

\begin{lstlisting}
def compute_domains(graph):
    domains = {}
    for v in range(graph.h):
        if graph.is_frozen(v):
            domains[v] = {6}       # gele : reste hexagone
        else:
            domains[v] = {5, 6, 7}  # libre : peut changer
    return domains
\end{lstlisting}

Un hexagone est gelé si :
\begin{itemize}
  \item son degré dans le dual est 6 (entouré de tous côtés), \textbf{ou}
  \item $b(v) \geq 2$ (les côtés partagés ne forment pas un bloc unique).
\end{itemize}

\subsection{Filtrage de la table de voisinage}

La table de voisinage $\mathcal{T}$ contient 1203 entrées réparties en :
$\mathcal{T}(5)$ = 92 entrées, $\mathcal{T}(6)$ = 412 entrées, $\mathcal{T}(7)$ = 699 entrées.
...
\textbf{Résultats :}
\begin{itemize}
  \item \texttt{first.graph} : \textbf{2/2 solutions planes}
...
csp_solver/
  ...
  utils/
    ...
    table.py                 # Chargement table voisinage (JSON, 1203 entrees)
...
\textbf{Commandes :}
\begin{lstlisting}[style=benzai]
python main.py data/first.graph               # Resoudre
