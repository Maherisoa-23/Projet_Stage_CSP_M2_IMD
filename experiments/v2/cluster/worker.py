"""
Worker cluster pour experiments_v2.

Duplique experiments/cluster/worker.py mais :
  - SCRATCH_PREFIX = "coala_v2_" (separation)
  - RUN_ONE_JOB pointe sur experiments_v2/run_one_job.py
  - propage entry["extra_flags"] vers run_one_job (--no-freeze etc)

Reste : pull-based, atomic claims sur NFS, ProcessPoolExecutor.

Usage : idem worker existant.
    python -m experiments.v2.cluster.worker \\
...
if __name__ == "__main__" and __package__ is None:
    ...
    __package__ = "experiments.v2.cluster"
...
# run_one_job.py de experiments_v2/, pas de experiments/.
...
def execute_job(entry, output_root, scratch_root, timeout_sec):
    ...
    # Propage les extras (no-freeze, no-table, adj-57) au run_one_job_v2
...
def worker_main():
    ...
    if not RUN_ONE_JOB.is_file():
        print(f"ERR run_one_job_v2 introuvable : {RUN_ONE_JOB}", file=sys.stderr)
        sys.exit(2)
    ...
    pid = os.getpid()
    print(f"=== Worker_v2 {host}:{pid} demarre ===", flush=True)
    ...
    print(f"\n=== Worker_v2 {host}:{pid} termine ({duration}s) ===", flush=True)
